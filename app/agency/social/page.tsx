...
function shareUrl(platformId: string, text: string) {
  const encoded = encodeURIComponent(text)
  const page = encodeURIComponent("https://ai-fellowship-nevil.vercel.app/agency")
  if (platformId === "x") return `https://twitter.com/intent/tweet?text=${encoded}`
  if (platformId === "reddit") return `https://www.reddit.com/submit?title=${encoded}&url=${page}`
  if (platformId === "facebook") return `https://www.facebook.com/sharer/sharer.php?u=${page}&quote=${encoded}`
  if (platformId === "substack") return "https://substack.com/home"
  return null
}
      ...
      >
        {step === "confirm" && (
          <>
            ...
            <p className="text-xs text-white/40 mb-5">
              Full account sync needs a {platform.name} developer app and OAuth credentials. Until then, Mastical uses one-click composer handoff where the platform supports it.
            </p>
            <div className="space-y-2 mb-5">
              {["One-click composer handoff", "Copy-ready post text", "Neural score tracking per post", "Clear OAuth setup path for later"].map((f) => (
        ...
        {step === "auth" && (
          <div className="text-center py-4">
            ...
            <p className="text-sm font-semibold text-white mb-1">OAuth credentials required</p>
            <p className="text-xs text-white/40 mb-4">Create a {platform.name} developer app, then wire its client id and secret into the posting API.</p>
            <motion.button
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
...
export default function SocialHubPage() {
  ...
  const publishNow = () => {
    ...
    if (!text || selectedPlatforms.length === 0) return

    const unsupported: string[] = []
    let opened = 0
    selectedPlatforms.forEach((platformId) => {
      const url = shareUrl(platformId, text)
      const platform = platformById[platformId]?.name || platformId
      if (url) {
    ...
    logActivity({
      kind: "publish",
      message: `Opened ${opened} composer${opened === 1 ? "" : "s"} from Social Hub`,
    })
    setPublishNote(
      unsupported.length
        ? `Post text copied. ${unsupported.join(", ")} needs manual paste because it does not support reliable web share intents.`
        : "Composer opened and post text copied."
    )
  }
          ...
          >
            ...
            >
              ...
              <div className="p-5">
                ...
                {publishNote && (
                  <p className="mt-3 text-[11px] text-emerald-300 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2">
                    {publishNote}
                  </p>
                )}
              </div>
