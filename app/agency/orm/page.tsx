...
export default function ORMPage() {
  ...
  useEffect(() => {
    ...
    setScanQuery(brand)
    setKeywords((prev) => (prev.includes(brand.toLowerCase()) ? prev : [brand.toLowerCase(), ...prev]))
    scanMentions(brand)
  }, [])
  ...
  const scanMentions = async (query = scanQuery) => {
    ...
    try {
      const settings = getSettings()
      const res = await fetch(`/api/agency/reputation?q=${encodeURIComponent(cleanQuery)}&brand=${encodeURIComponent(settings.defaultBrand || cleanQuery)}`)
  ...
  const reputationScore = totalMentions
    ? Math.max(0, Math.min(100, Math.round(((sentimentCounts.positive * 1 + sentimentCounts.neutral * 0.65) / totalMentions) * 100)))
    : 0
  const weekChange = totalMentions ? `+${totalMentions}` : "0"
  const sentimentData = totalMentions
    ? [
        { name: "Positive", value: sentimentCounts.positive, color: "#10b981" },
        { name: "Neutral", value: sentimentCounts.neutral, color: "#64748b" },
        { name: "Negative", value: sentimentCounts.negative, color: "#ef4444" },
      ].filter((item) => item.value > 0)
    : [{ name: "No mentions", value: 1, color: "#334155" }]
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={isScanning}
              className="flex items-center gap-2 px-3 py-2 rounded-lg border border-white/[0.07] text-xs text-white/40 hover:text-white/70 disabled:opacity-40 transition-colors"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="col-span-1 bg-[#0f0f1a] border border-emerald-500/15 rounded-xl p-5 flex flex-col items-center"
            ...
            <div className="mt-3 text-center">
              <p className="text-[10px] text-white/30">Based on {totalMentions} live mentions</p>
          ...
          <div className="col-span-1 bg-[#0f0f1a] border border-white/[0.06] rounded-xl p-5">
            ...
            <div className="h-28">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={sentimentData} cx="50%" cy="50%" innerRadius={32} outerRadius={50}
          ...
          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              { label: "Total Mentions", value: totalMentions.toString(), sub: `+${weekChange} this week`, icon: MessageCircle, color: "text-violet-400", trend: true },
                  { label: "Positive Mentions", value: sentimentCounts.positive.toString(), sub: totalMentions ? `${Math.round((sentimentCounts.positive / totalMentions) * 100)}% of all mentions` : "No public mentions yet", icon: ThumbsUp, color: "text-emerald-400", trend: true },
              { label: "Needs Response", value: `${filteredMentions.filter((m) => !replied.has(m.id) && m.priority === "high").length}`, sub: "High priority unanswered", icon: AlertTriangle, color: "text-red-400", trend: false },
                  { label: "Avg Reach / Mention", value: totalMentions ? `${Math.round(mentions.reduce((sum, m) => sum + m.reach, 0) / totalMentions)}` : "0", sub: "Reddit votes/comments + HN points/comments", icon: Eye, color: "text-cyan-400", trend: true },
            ].map((s) => {
              const Icon = s.icon
              return (
                                ...
                                <div className="bg-[#0a0a14] border border-violet-500/20 rounded-xl p-3">
                                  <div className="flex flex-wrap gap-1.5 mb-2">
                                    {REPLY_TEMPLATES.slice(0, 2).map((t, i) => (
                                      <button
                                        key={i}
                                      onClick={() => setReplyText(i === 0 && mention.draftReply ? mention.draftReply : t)}
